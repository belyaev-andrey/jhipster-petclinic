...
import io.jh.petclinic.config.EmbeddedKafka;
import io.jh.petclinic.config.KafkaSseConsumer;
import io.jh.petclinic.config.KafkaSseProducer;
...
@WithMockUser
@EmbeddedKafka
class JhpetclinicKafkaResourceIT {
    ...
    void producesMessages() throws Exception {
        restMockMvc.perform(post("/api/jhpetclinic-kafka/publish?message=value-produce")).andExpect(status().isOk());
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc
            .perform(get("/api/jhpetclinic-kafka/register"))
        ...
        for (int i = 0; i < 100; i++) {
            ...
            if (content.contains("data:value-consume")) {
                restMockMvc.perform(get("/api/jhpetclinic-kafka/unregister"));
                return;
            }
        }
